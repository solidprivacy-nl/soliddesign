...
def render_print_pack(
    ...
    *,
    ...
    if audit.current_screenshot_data_uri:
        current_html = f'<img src="{html.escape(audit.current_screenshot_data_uri, quote=True)}" alt="Huidige website">'
    else:
        current_html = '<div class="placeholder">Huidige screenshot wordt bij live audit toegevoegd</div>'
...
<div class="compare"><div class="panel"><h3>HUIDIGE WEBSITE</h3>{current_html}</div><div class="panel"><h3>CONCEPT</h3>{concept_svg}</div></div>
<h2>Drie kansen die ons opvielen</h2><ul>{opportunity_items}</ul>
